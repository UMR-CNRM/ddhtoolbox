...
real(kind=8), pointer :: zx(:),zxloc(:)
real(kind=8), pointer :: zy(:),zyloc(:)
real(kind=8), pointer :: zu(:),zuloc(:)
real(kind=8), pointer :: zv(:),zvloc(:)
real(kind=8), pointer :: zc(:),zcloc(:)
...
if(lllfa) then
  ...
  !-------------------------------------------------
  !
  call lfalecgen(cgfdta(1),zxloc,zyloc,zuloc,zvloc,zcloc,indta)
else
  !
  !-------------------------------------------------
  ...
  !-------------------------------------------------
  !
  write(*,fmt=*) '  Lecture du fichier ASCII ',trim(cgfdta(1))
...
!-------------------------------------------------
!
if(.not. lllfa) iule=22 ; open(iule,file=cgfdta(1),form='formatted')
!
!-------------------------------------------------
! Lecture séquentielle.
!-------------------------------------------------
...
do
  if(lllfa) then
    ...
    zutmp=zuloc(indta_lfa)
    zvtmp=zvloc(indta_lfa)
