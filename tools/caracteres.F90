...
! --------------------------------------------------------------------------
implicit none
INTEGER(KIND=4) :: KPOS1
INTEGER(KIND=4) :: KPOS2
INTEGER(KIND=4) :: KRES
...
INTEGER(KIND=4) :: IC
INTEGER(KIND=4) :: IMOT
INTEGER(KIND=4) :: IPRESMOT
INTEGER(KIND=4) :: JC
INTEGER(KIND=4) :: KCRIM
INTEGER(KIND=4) :: KMOT
...
subroutine minmaj_debut_mot(cdce,cdcs)
! --------------------------------------------------------------
! **** *minmaj_debut_mot* Passage minuscules > majuscules en début de chaque mot.
! --------------------------------------------------------------
! Sujet:
! Arguments explicites:
! Arguments implicites:
...
! Modifications:
! --------------------------------------------------------------
! En entree: cdce; exemple: 'MON chien est ici'
! En sortie: cdcs; exemple: 'Mon Chien Est Ici'
! --------------------------------------------------------------
implicit none
INTEGER(KIND=4) :: JC
character*(*) cdce, cdcs
character*1 cl1,clprec
cdcs=' '
do jc=1,len_trim(cdce)
  cl1=cdce(jc:jc)
  if(jc == 1) then
    !
    !-------------------------------------------------
    ! Début de ligne; on met en majuscules.
    !-------------------------------------------------
    !
    if(cl1 >= 'a'.and.cl1 <= 'z') cl1=char(ichar(cl1)-32) ; cdcs(jc:jc)=cl1
  else
    !
    !-------------------------------------------------
    ! Intérieur de ligne; on met en majuscules si on est en début de mot.
    !-------------------------------------------------
    !
    clprec=cdce(jc-1:jc-1)
    if(clprec == ' ' .or. clprec == '-') then
      !
      !-------------------------------------------------
      ! On est en début de mot > majuscules.
      !-------------------------------------------------
      !
      if(cl1 >= 'a'.and.cl1 <= 'z') cl1=char(ichar(cl1)-32) ; cdcs(jc:jc)=cl1
    else
      !
      !-------------------------------------------------
      ! On est en intérieur de mot > minuscules.
      !-------------------------------------------------
      !
      if(cl1 >= 'A'.and.cl1 <= 'Z') cl1=char(ichar(cl1)+32) ; cdcs(jc:jc)=cl1
    endif
...
! --------------------------------------------------------------------------
implicit none
INTEGER(KIND=4) :: JTEST
INTEGER(KIND=4) :: KLSOR
...
! --------------------------------------------------------------
implicit none
INTEGER(KIND=4) :: IDEBRECH
INTEGER(KIND=4) :: ILMOT
INTEGER(KIND=4) :: IPOS
INTEGER(KIND=4) :: JMOT
INTEGER(KIND=4) :: KLMOT
INTEGER(KIND=4) :: KMOT

character*(*) cdmot(klmot)
character*(*) cdc
INTEGER(KIND=4) kdebmot(klmot)
INTEGER(KIND=4) kfinmot(klmot)
...
! --------------------------------------------------------------------------
implicit none
INTEGER(KIND=4) :: IDEB1
INTEGER(KIND=4) :: IDEB2
INTEGER(KIND=4) :: IFIN1
INTEGER(KIND=4) :: IFIN2
INTEGER(KIND=4) :: IL1
INTEGER(KIND=4) :: IL2
INTEGER(KIND=4) :: JC
INTEGER(KIND=4) :: KDEB1
INTEGER(KIND=4) :: KDEB2
INTEGER(KIND=4) :: KFIN1
INTEGER(KIND=4) :: KFIN2
INTEGER(KIND=4) :: KREP
...
if(il1 > il2) return
do jc=0,il2-il1
  if(cd1(ideb1:ifin1) == cd2(ideb2+jc:ideb2+jc+il1-1)) then
    krep=ideb2+jc
    goto 200
  endif
enddo
...
implicit none
INTEGER(KIND=4) :: IAFF
INTEGER(KIND=4) :: IAFFTMP
INTEGER(KIND=4) :: ILOG
INTEGER(KIND=4) :: ILOG2
INTEGER(KIND=4) :: ILONLOG
INTEGER(KIND=4) :: ININT
INTEGER(KIND=4) :: ISIGN
INTEGER(KIND=4) :: IX
INTEGER(KIND=4) :: JCH
INTEGER(KIND=4) :: KNC
...
subroutine extb(cdnom,ktype,cdsig,klsig,cdsig2,klsig2)
! --------------------------------------------------------------------------
! **** *EXTB* Elimination des blancs et tabulations d'une chaine.
! --------------------------------------------------------------------------
! Sujet:
! ------
! Arguments explicites:
...
!
! '      W    Zx = 9.'      'zx=9.'           'Zx = 9.'
! '       proGRAm   i'      'programi'        'proGRAm   i'
!
! --------------------------------------------------------------------------
implicit none
INTEGER(KIND=4) :: ICAR
INTEGER(KIND=4) :: IDEBUT
INTEGER(KIND=4) :: IFIN
INTEGER(KIND=4) :: ILNOM
INTEGER(KIND=4) :: IPREMNB
INTEGER(KIND=4) :: JC
INTEGER(KIND=4) :: KLSIG
INTEGER(KIND=4) :: KLSIG2
...
if(ktype == 1) then
  ...
  if(cdnom(1:1) == '!') then
    idebut=2
  elseif(cdnom(1:1) == '	'.or.cdnom(1:1) == ' ') then
    idebut=0
    do jc=2,ilnom
      if(cdnom(jc:jc) == '!'.and.idebut == 0) then
        idebut=jc+1
      endif
    enddo
  else
...
elseif(ktype == 3) then
  ...
  print*,'ERREUR interne extb!...'
  stop
endif
ipremnb=0 ! indicateur de premier non blanc.
do jc=idebut,ilnom
  cl1=cdnom(jc:jc)
  if(cl1 /= ' '.and.cl1 /= '	') then
    if(cl1 >= 'A'.and.cl1 <= 'Z') then
      cl1=char(ichar(cl1)+32)
    endif
    klsig=klsig+1
    cdsig(klsig:klsig)=cl1
    ipremnb=1
  endif
  cl1=cdnom(jc:jc)
  if((cl1 /= ' '.and.cl1 /= '	').or.ipremnb /= 0) then
    klsig2=klsig2+1
    cdsig2(klsig2:klsig2)=cl1
  endif
enddo
if(ktype == 2.or.ktype == 3) then
  !
  ! Dans le cas des lignes executables ou suites
  ! on elimine de la chaine cdsig les eventuels
  ! commentaires.F90 de fin de ligne.
  !
  icar=0 ! indicateur d'interieur de chaine de caracteres.
  ifin=klsig ! position du dernier caractere non de commentaire.
  do jc=1,klsig
    if(cdsig(jc:jc) == '''') icar=1-icar
    if(cdsig(jc:jc) == '!'.and.icar == 0) ifin=jc-1
  enddo
  klsig=ifin
endif
end
