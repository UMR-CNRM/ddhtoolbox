...
INTEGER(KIND=4) :: IAN,IMO,IQU
REAL(KIND=8) :: PSSSSS,PSTATI
REAL(KIND=8) :: ZSSSSS,ZDJ,ZSV,ZFRAC,ZSVV
REAL(KIND=8) :: ZECH
...
!
ZFRAC=PSSSSS/3600.
IHE=INT(ZFRAC) ! heure de la base.
ZFRAC=(ZFRAC-REAL(IHE))*60.
IMI=INT(ZFRAC) ! minute de la base.
ZFRAC=60.*(ZFRAC-REAL(IMI))
IF(ZFRAC > 59.999) THEN
  !
  !-------------------------------------------------
  ! Cas des erreurs d'arrondi de la machine: si on est très proche de la minute
  ! suivante (à moins d'une seconde près, i.e. le nombre de secondes est 59.999), 
  ! on passe à la minute suivante.
  !-------------------------------------------------
  !
  ZFRAC=60.0001
ENDIF
CALL AMQHMS_VERS_DJ(KAN,KMO,KQU,IHE,IMI,ZFRAC,ZDJ)
CALL DJ_VERS_AMQHMS(ZDJ,IAN,IMO,IQU,IHE,IMI,ZSV) ! date grégorienne de la base.
ZDJ=ZDJ+PSTATI/86400. ! date julienne de validité.
CALL DJ_VERS_AMQHMS(ZDJ,IANV,IMOV,IQUV,IHEV,IMIV,ZSVV) ! date grégorienne de validité.
!
!-------------------------------------------------
...
IF(IMI == 0 .AND. IMIV == 0) THEN
  ...
  WRITE(CDTIT,FMT='(3a,i2,a,i2.2,a,i4.4,a,i2.2,5a,i2,a,i2.2,a,i4.4,a,i2.2,a)')&
    &'Analysis ',TRIM(CLJS),' ',IQU,'.',IMO,'.',IAN,' ',IHE,'h UTC + ',CLECH(1:LEN_TRIM(CLECH))&
...
ELSE
  ...
  WRITE(CDTIT,FMT='(3a,i2,a,i2.2,a,i4.4,a,i2.2,a,i2.2,5a,i2,a,i2.2,a,i4.4,a,i2.2,a,i2.2,a)')&
    &'Analysis ',TRIM(CLJS),' ',IQU,'.',IMO,'.',IAN,' ',IHE,':',IMI,' UTC + ',CLECH(1:LEN_TRIM(CLECH))&
...
!-------------------------------------------------
!
PDJ=2451545.0_8- 0.5_8 +REAL(IECART)+REAL(KHH)/24._8 &
  & +REAL(KMN)/1440._8+PS/86400._8
END
...
IF(PS > 59.999) THEN
  !
  !-------------------------------------------------
  ! Cas d'erreur d'arrondi machine. On passe à la minute suivante.
  !-------------------------------------------------
  !
  PS=0.
  KMN=KMN+1
  IF(KMN == 60) THEN
    ...
    IF(KHH == 24) THEN
      IEC=1
      CALL DAPLUSJ(IDATE2,IEC,IDATEARR)
      KQQ=MOD(IDATEARR,100)
      KNOUV=IDATEARR/100
      KMM=MOD(KNOUV,100)
      KAAAA=KNOUV/100
    ENDIF
