...
!-------------------------------------------------
!
character*2000, parameter :: cgindef='jhfjkefzref%6'
character*2000 :: cgfnom(jpmaxul)=cgindef
end
function iargcp()
! --------------------------------------------------------------
...
character*(*) cdna
character*2000 cldta(klbouc)
call lfalecc(kul,cdna,klbouc,cldta,ilong,ierr)
do jlong=1,ilong
  write(*,'(a)') trim(cldta(jlong))
enddo
end
...
character(len=2000) :: clint
call lfaleci(kul,cdna,klbouc,idta,ilong,ierr)
do jlong=1,ilong
  write(clint,fmt=*) idta(jlong)
  write(*,fmt='(a)') trim(adjustl(clint))
enddo
end
...
character*(*) cdnae, cdnas
character*2000 clnas
character*2000 clbouc(klong)
!
...
&   ,ilnom,ilmin,ilmax
character*2000 cldonmin,cldonmax
character*2000 cldon(klong)
character*2000 cdna
character*(*) cdtype
...
character*2000 clmin,clmax,clmoy,clrcm,cllong
character*2000 cdna
character*(*) cdtype
...
if(lglang) then
  if(ilong >= 10000000) then
    write(*,fmt=*) 'l=',ilong,', min= ',imin,' max= ',imax,' moy= ',zmoy,' rcm= ',zrcm ,'|',cdtype,'| ',cdna(1:ilnom)
  else
    write(*,'(a,i8,2(a,i11),2(a,g11.4),4a)') 'l=',ilong,', min= ',imin,' max= ',imax,' moy= ',zmoy,' rcm= ',zrcm ,'|',cdtype,'| ',cdna(1:ilnom)
  endif
else
  if(ilong >= 10000000) then
    write(*,fmt=*) 'l=',ilong,', min= ',imin,' max= ',imax,' mea= ',zmoy,' rms= ',zrcm,'|',cdtype,'| ',cdna(1:ilnom)
  else
    write(*,'(a,i8,2(a,i11),2(a,g11.4),4a)') 'l=',ilong,', min= ',imin,' max= ',imax,' mea= ',zmoy,' rms= ',zrcm,'|',cdtype,'| ',cdna(1:ilnom)
  endif
endif
end
...
character*2000 clmin,clmax,clmoy,clrcm
character*2000 cdna
character*(*) cdtype
...
character*2000 clnomf,classign
#include"lfayom.h"
if(nmes(kul) == 2) then
  !
  ! Messagerie bavarde.
...
&   ,iulout
character*2000 clnomf,clnarc,clnarl,clnaec,clnael,clnac,clnara &
&   ,clnard,clnaed
...
logical llex
character*2000 clfs,clnoma,clvalfic,cltype
character*3 cllang
!
! Saisie de la ligne de commande.
!
...
logical llinter
character*2000 clfe1,clfe2,clfs
parameter(jplis=1800) ! nombre maxi d'articles dans les fichiers LFA d'entree.
character*2000 cllis1(jplis) ! liste des articles du fichier d'entree 1.
character*2000 cllis2(jplis) ! liste des articles du fichier d'entree 2.
character*3 cllang
!
! Saisie de la ligne de commande.
!
...
subroutine lfapplfa2dd
! --------------------------------------------------------------------------
! **** *LFAPPLFA2DD* Rédaction d'une autodocumentation en vue de tracé.
! --------------------------------------------------------------------------
! Sujet:
! ------
! Arguments explicites:
...
! Externes:
! ---------
! Auteur:   2020-04, J.M. Piriou.
! -------
! Modifications:
! --------------------------------------------------------------------------
implicit none
#include"lfatail.h"
character*2000 clnomf,clfdd,cltype
integer(kind=jpintusr) iul,inarg,ierrL,ierrX,ierrY,ierrU,ilong,iullfa1,ierrV,iarg
logical lllfa
character*3 cllang
!
! Ouverture du fichier.
!
inarg=1 ; call getargp(inarg,clnomf)
inarg=2 ; call getargp(inarg,clfdd)
iarg=iargc()
if(iarg /= 2) then
  print*,' '
  print*,'Programme reconnaissant des champs XYV LLV XYUVC etc dans un fichier LFA et écrivant l''autodocumentation associée en vue de tracé.'
  print*,' '
  print*,'Utilisation: lfa2dd FLFA FDOC'
  print*,' '
  stop
endif
iul=7
...
if(lllfa) then
  ...
  !
  iullfa1=23
  call lfaouv(iullfa1,clnomf,'R')
  !
  !-------------------------------------------------
  ! Test d'existence d'un article donné.
  !-------------------------------------------------
  !
  call lfacas(iullfa1,'X',cltype,ilong,ierrX)
  call lfacas(iullfa1,'Y',cltype,ilong,ierrY)
  call lfacas(iullfa1,'U',cltype,ilong,ierrU)
  call lfacas(iullfa1,'V',cltype,ilong,ierrV)
  call lfacas(iullfa1,'LONGITUDE',cltype,ilong,ierrL)
  if(ierrX /= 0 .and. ierrY /= 0 .and. ierrU /= 0 .and. ierrX /= 0) then
    stop
  else
    !
    !-------------------------------------------------
    ...
    !-------------------------------------------------
    !
    iul=40 ; open(iul,file=clfdd,form='formatted')
    if(ierrX == 0 .and. ierrY == 0 .and. ierrV == 0) then
      write(iul,fmt='(9a)') '#FORMAT=LLV'
    elseif(ierrL == 0) then
      write(iul,fmt='(9a)') '#FORMAT=LLV'
    elseif(ierrX == 0 .and. ierrY == 0 .and. ierrU == 0 .and. ierrV ==0) then
      write(iul,fmt='(9a)') '#FORMAT=XYUVC'
    else
      write(*,fmt=*)
      write(*,fmt=*) 'lfa2dd/ERREUR: articles non reconnus !...'
      write(*,fmt=*)
      call exit(1)
    endif
    write(iul,fmt='(2a)') '#FICHIER=',trim(clnomf)
    write(iul,fmt='(2a)') '#TITRE=',trim(clnomf)
    write(iul,fmt='(2a)') '#DATE='
    write(iul,fmt='(2a)') '#UNITE='
    close(iul)
  endif
endif
end
...
character*2000 clarg,clna,clfent
character*3 cllang
!
!-------------------------------------------------
...
implicit none
#include"lfatail.h"
character*2000 clnomf,clnomfs,clna
integer(kind=jpintusr) iul,iuls,ilong,ierr,ilnomf,inarg
character*2 cltype
character*3 cllang
...
interface lfalecgen
  !
  !-------------------------------------------------
  ! La présente interface permet de choisir entre la lecture XYV, XYUVC, etc, en appelant toujours le même générique lfalecgen; c'est le logiciel LFA qui choisit l'appel aux routines spécifiques lfalecxyv, etc, en fonction du nombre d'arguments d'appel au générique lfalecgen.
  !-------------------------------------------------
  !
  module procedure lfalecxyv,lfalecxyuvc
end interface
contains
subroutine lfalecxyv(cdlfa,px,py,pv,klen)
! --------------------------------------------------------------------------
! **** *LFALECXYV* Lecture de reels sur fichier LFA.
! **** *LFALECXYV* Read real data on LFA file.
! --------------------------------------------------------------------------
! Sujet:
! ------
! Arguments explicites:
...
! --------------------------------------------------------------------------
! En entree:
! cdlfa            fichier LFA d'entrée.
! En sortie:
! px,py,pv         tableaux de données lus.
! klen             nombre de valeurs dans px, py ou pv.
! --------------------------------------------------------------------------
implicit none
#include"lfatail.h"
character(len=*), intent(in) :: cdlfa
real(kind=jpreeusr), dimension(:), pointer :: px
real(kind=jpreeusr), dimension(:), pointer :: py
real(kind=jpreeusr), dimension(:), pointer :: pv
integer(kind=jpintusr), intent(out) :: klen

integer(kind=jpintusr) :: iullfa1,lfa_freeu,ierr,ilong
character(len=2000) :: cltype,clnax,clnay,clnav
#include"lfayom.h"
#include"lfadoc.h"
!
!-------------------------------------------------
...
!-------------------------------------------------
!
call lfaouv(iullfa1,cdlfa,'R')
!
!-------------------------------------------------
! Recherche du nom de l'abscisse: X ou LONGITUDE?.
! On attend des noms d'article de type X Y V, ou alors LONGITUDE LATITUDE FIELD.
!-------------------------------------------------
!
clnax='X'
call lfacas(iullfa1,clnax,cltype,klen,ierr)
if(ierr == 0) then
  !
  !-------------------------------------------------
  ! Le fichier LFA contient des articles de nom X Y V.
  !-------------------------------------------------
  !
  clnay='Y'
  clnav='V'
  ...
  if(ierr == 0) then
    !
    !-------------------------------------------------
    ! Le fichier LFA contient des articles de nom LONGITUDE LATITUDE FIELD.
    !-------------------------------------------------
    !
    clnay='LATITUDE'
    clnav='FIELD'
    ...
    write(*,fmt=*)
    write(*,fmt=*) 'lfalecxyv/ERREUR: nom d''article de la coordonnée X inconnu !...'
    write(*,fmt=*)
    call exit(1)
  endif
endif
...
write(*,fmt=*) '  lfalecgen: lecture de ',trim(clnax),' ',trim(clnay),' ',trim(clnav),' dans le fichier LFA ',trim(cdlfa),', ',klen,' valeurs chacun.'

call lfalecr(iullfa1,clnax,klen,px,ilong,ierr)
call lfalecr(iullfa1,clnay,klen,py,ilong,ierr)
call lfalecr(iullfa1,clnav,klen,pv,ilong,ierr)
!
!-------------------------------------------------
...
subroutine lfalecxyuvc(cdlfa,px,py,pu,pv,pc,klen)
! --------------------------------------------------------------------------
! **** *LFALECXYUVC* Lecture de reels sur fichier LFA.
! **** *LFALECXYUVC* Read real data on LFA file.
! --------------------------------------------------------------------------
! Sujet:
! ------
! Arguments explicites:
...
real(kind=jpreeusr), dimension(:), pointer :: px
real(kind=jpreeusr), dimension(:), pointer :: py
real(kind=jpreeusr), dimension(:), pointer :: pu
real(kind=jpreeusr), dimension(:), pointer :: pv
real(kind=jpreeusr), dimension(:), pointer :: pc
integer(kind=jpintusr), intent(out) :: klen

integer(kind=jpintusr) :: iullfa1,lfa_freeu,ierr,ilong
character(len=2000) :: cltype,clnax,clnay,clnau,clnav,clnac
#include"lfayom.h"
#include"lfadoc.h"
!
!-------------------------------------------------
...
if(ierr == 0) then
  ...
  if(ierr == 0) then
    ...
    write(*,fmt=*)
    write(*,fmt=*) 'lfalecxyuvc/ERREUR: nom d''article de la coordonnée X inconnu !...'
    write(*,fmt=*)
    call exit(1)
  endif
endif
...
write(*,fmt=*) '  lfalecgen: lecture de ',trim(clnax),' ',trim(clnay),' ',trim(clnau),' ',trim(clnav),' ',trim(clnac),' dans le fichier LFA ',trim(cdlfa),', ',klen,' valeurs chacun.'

call lfalecr(iullfa1,clnax,klen,px,ilong,ierr)
call lfalecr(iullfa1,clnay,klen,py,ilong,ierr)
call lfalecr(iullfa1,clnau,klen,pu,ilong,ierr)
call lfalecr(iullfa1,clnav,klen,pv,ilong,ierr)
call lfalecr(iullfa1,clnac,klen,pc,ilong,ierr)
!
!-------------------------------------------------
! Fermeture du fichier LFA.
!-------------------------------------------------
...
IMPLICIT NONE
#include"lfatail.h"
integer(kind=jpintusr) :: lfa_freeu,junit
logical*4 llopen
do junit=25,200
...
subroutine lfaecrxyv(cdlfa,px,py,pv,klen)
! --------------------------------------------------------------------------
! **** *LFAECRXYV* Ecriture de reels sur fichier LFA.
! **** *LFAECRXYV* Write real data on LFA file.
! --------------------------------------------------------------------------
! Sujet:
! ------
! Arguments explicites:
...
! klen             nombre de valeurs dans px, py ou pv.
! En sortie:
! Ecriture d'un fichier LFA.
! --------------------------------------------------------------------------
implicit none
#include"lfatail.h"
character(len=*), intent(in) :: cdlfa
real(kind=jpreeusr), intent(in) :: px(klen)
real(kind=jpreeusr), intent(in) :: py(klen)
real(kind=jpreeusr), intent(in) :: pv(klen)
integer(kind=jpintusr), intent(in) :: klen
...
!-------------------------------------------------
!
call lfaouv(iullfa1,cdlfa,'W')
call lfaprecr(iullfa1,4)
call lfaecrr(iullfa1,'X',px,klen)
call lfaecrr(iullfa1,'Y',py,klen)
call lfaecrr(iullfa1,'V',pv,klen)
!
!-------------------------------------------------
! Fermeture du fichier LFA.
!-------------------------------------------------
!
write(*,fmt=*) 'lfaecrxyv : écriture du fichier LFA ',trim(cdlfa)
call lfaminm(iullfa1)
call lfafer(iullfa1)
end
#include"cllang.F90"
